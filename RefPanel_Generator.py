...
#endouthapfile = subprocess.check_output(command, shell = True)
#print("Before filtering: " + end_hap + "\nAfter filtering: "+ endouthapfile)
#Change the value of the last row in the file
#end_hap = endouthapfile[:-1]

#Ancentral population will be given in argument 7 onwards
for i in range(n):
	indx = i + 8
	pop = sys.argv[indx]
	#Extract $num position from pop_i
	outpop = num + "_" + pop + "_positions.txt" 
	command = "grep -n " + pop + " " + ancpop + " | cut -d ':' -f1 | head -n " +  num + " > "  + outpop
	subprocess.call (command, shell=True)
	...
	for j in range(m):
		...
		e = int(e[:-1]) + 8
		#Get the data
		outpop_rescued = num + "_" + pop + "_rescued_cut.txt" 
		command = "awk 'NR>=" + sta_hap +"&&NR<="+ end_hap +"{print}NR>="+ end_hap +"{exit}' " + outhapfile +  "| cut -f" + str(s) + "-" + str(e) + "|  tr '|' ' ' | tr '\\t' ' '  > " + outpop_rescued
		subprocess.call (command, shell=True)
		#If ranges are given in more than one line
		if j > 1:
			#Temporal file to save ranges, if needed
			command = "awk 'NR>=" + sta_hap +"&&NR<="+ end_hap +"{print}NR>="+ end_hap +"{exit}' " + outhapfile +  "| cut -f" + str(s) + "-" + str(e) + "|  tr '|' ' ' | tr '\\t' ' '  >  temporal_rescued_cut.txt"
			subprocess.call (command, shell=True)
			#The paste command allows the column concatenation needed in this case
			command = "paste -d ' ' " + outpop_rescued + " temporal_rescued_cut.txt > " + outpop_rescued	
...
ref_pan = id + "_" + num + "_full_reference_panel.txt"
for i in range(n):
	indx = i + 8
	pop = sys.argv[indx]
	outpop_rescued = num + "_" + pop + "_rescued_cut.txt"
	...
	#A >> generates a file with more rows than the ones rescued. By substituting the file, the number of rows are preserved.
		command = "paste -d ' ' " +ref_pan + " " + outpop_rescued + " > " + ref_pan
